...
class ApiService {
    ...
    /**
     * (POST) vanage 전송 Service
     */

    async sendVonage(req) {
        try {
            const vonage = new Vonage({
                apiKey: "f7318ea", //f7318eab
                apiSecret: "OWHa2PAx4CJoljJq" //OWHa2PAx4CJoljJq
            })
            let status = 100
            await vonage.sms.send(req)
                .then(resp => {
                    console.log('Message sent successfully'); console.log(resp);
                    status = resp.messages[0].status
                })
                .catch(err => {
                    console.log('There was an error sending the messages.'); console.log(err.response);

                    if (err.response.messages) {
                        status = err.response.messages[0].status
                    } else {
                        status = err.code
                    }
                });
